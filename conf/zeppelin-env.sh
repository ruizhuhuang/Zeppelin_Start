...
export ZEPPELIN_LOG_DIR=$DATA/.zeppelin/logs        		# Where log files are stored.  PWD by default.
export ZEPPELIN_PID_DIR=$DATA/.zeppelin/run        		# The pid files are stored. ${ZEPPELIN_HOME}/run by default.
export ZEPPELIN_WAR_TEMPDIR=$DATA/.zeppelin/webapps    		# The location of jetty temporary directory.
...
export ZEPPELIN_INTERPRETER_LOCALREPO=$DATA/.zeppelin/local-repo         # Local repository for interpreter's additional dependency loading
...
export SPARK_SUBMIT_OPTIONS="--master yarn --deploy-mode client --driver-memory 4g  --executor-memory 2g --num-executors 12 --executor-cores 2"          # (optional) extra options to pass to spark submit. eg) "--driver-memory 512M --executor-memory 1G".
